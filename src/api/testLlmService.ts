...
async function generateSOP(data: any): Promise<string> {
  try {
    return await llmApi.generateSOP(data);
  } catch (error) {
...
async function generateLOR(data: any): Promise<string> {
  try {
    return await llmApi.generateLOR(data);
  } catch (error) {
