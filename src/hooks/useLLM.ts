...
export function useGenerateStatementOfPurpose(demoMode = false, applicationId?: Id<"applications">) {
  console.log("[useGenerateStatementOfPurpose] called with", { demoMode, applicationId });
  const applicationDetails = useQuery(
    api.applications.queries.getApplicationDetails,
    applicationId ? { applicationId, demoMode } : "skip"
  );
        ...
        if (sop) {
          ...
          if (sopDoc && sopDoc.id) {
            await saveDocumentDraft({
              applicationDocumentId: sopDoc.id,
              content: sop,
              demoMode
            });
          } else {
