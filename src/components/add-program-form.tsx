...
const programNameField: FieldDefinition = {
    ...
    label: "Program Name",
    description:
        "Enter the area of study. Do not include the degree type (e.g. MS, PhD, etc.)",
    renderField: (field) => (
        <FormControl>
  ...
  const greField: FieldDefinition = {
    ...
    renderField: (field) => (
      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
        ...
        <FormControl>
          <Checkbox checked={field.value} onCheckedChange={field.onChange} className="m-2" />
        </FormControl>
      </FormItem>
    ),
  };
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      const programId = await createProgram({
        ...
        name: data.name,
        degree:
          data.degree === "Other" && data.customDegree
            ? data.customDegree
            : data.degree,
      ...
      toast.success("Program created", {
        description:
          "Your program has been successfully added to our database.",
      });
      ...
      toast.error("Error", {
        description:
          "There was a problem creating your program. Please try again.",
      });
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="p-0">
        ...
        <DialogFooter className="px-6 py-4 border-t">
          <Button type="submit" form="add-program-form" disabled={isSubmitting}>
