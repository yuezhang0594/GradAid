...
export default function Dashboard() {
  ...
  const { applicationStats, documentStats, applicationTimeline } =
    useDashboardData();
  ...
  const handleDocumentClick = (doc: any) => {
    navigate(
      `/documents/${encodeURIComponent(doc.university)}/${doc.type.toLowerCase()}?documentId=${doc.documentId}`,
      {
        state: {
    ...
    <main className="flex-1 flex-col overflow-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="mt-6">
        ...
        {documentStats.length === 0 && (
          <Card className="text-center py-16">
            <CardTitle>You haven't started any applications yet.</CardTitle>
            <CardDescription>
              You can start a new application on the 'Apply' or 'Saved Programs'
              pages.
            </CardDescription>
            <CardContent>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {documentStats
            .slice(
              0,
              showAllDocuments ? documentStats.length : initialDocumentsToShow
            )
            .map((document, index) => (
              <ClickableCard
                key={index}
              ...
              >
                <CardHeader>
                  ...
                  <CardDescription className="flex flex-col space-y-1 text-left">
                    <span className="truncate max-w-[200px] text-xs text-muted-foreground text-left">
                      {document.university}
                    </span>
                    <span className="truncate max-w-[200px] text-xs text-muted-foreground text-left">
                      {document.program}
            ...
            >
              {showAllDocuments ? (
                ...
                <>
                  ...
                  Show More ({documentStats.length -
                    initialDocumentsToShow}{" "}
                  more)
                </>
              )}
            </Button>
      ...
      <div className="mt-6">
        ...
        {applicationTimeline.length === 0 ? (
          <Card className="text-center py-16">
            <CardTitle>No timeline events found.</CardTitle>
          </Card>
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {applicationTimeline
                .slice(
                  0,
                  showAllTimelines
                    ? applicationTimeline.length
                    : initialTimelinesToShow
                )
                .map((event, index) => (
                  <Card
                    key={index}
