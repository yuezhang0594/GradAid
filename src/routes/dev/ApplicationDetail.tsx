...
import { DocumentType } from "convex/validators";
...
export default function ApplicationDetail() {
  ...
  const { applicationData, applicationStats, createDocument, documentStats, isLoading } = useApplicationDetail(
    state?.applicationId ?? ""
  );
  const icons = {
  ...
  return (
    <PageWrapper
      title={applicationData.university}
      description={
        <div className="flex flex-col items-center text-center">
          <p className="text-lg">{`${applicationData.degree} in ${applicationData.program}`}</p>
    ...
    >
      ...
      <div className="mt-8">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          {/* Add "Create Statement of Purpose" card if no SOP exists */}
          {!documentStats.some(doc => doc.type === "sop") && (
            <ClickableCard
              key="create-sop"
              action={{
                ...
                href: "#",
                onClick: () => {
                  handleNewDocumentClick(applicationData._id, applicationData.university, "sop");
                }
              }}
            >
              <CardHeader>
                <div className="flex justify-between items-center">
                  <CardTitle className="text-sm group-hover:text-primary transition-colors text-left">Statement of Purpose</CardTitle>
                  <Badge variant="secondary" className="ml-2 hidden xl:inline-flex">
                    <span className="mr-1">+</span>
                    Create New
                  </Badge>
              ...
              <CardContent className="pt-8">
                <div className="space-y-2">
                  ...
                  <Progress value={0} className="h-2 group-hover:bg-primary/20 transition-colors" />
                </div>
              </CardContent>
            </ClickableCard>
          )}
