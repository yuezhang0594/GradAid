...
describe("createAiCredits", () => {
  ...
  test("should create an aiCredits record with custom values", async () => {
    ...
    const customResetDate = new Date("2025-12-31T00:00:00.000Z").toISOString();
...
describe("updateUserAiCredits", () => {
  ...
  test("should update the usedCredits for a valid aiCreditId", async () => {
    await t.run(async (ctx) => {
      await AiCreditsModel.updateUserAiCredits(ctx, testUserId, creditsToUse);
    });
    ...
    expect(updatedRecord?.usedCredits).toBe(startingUsedCredits + creditsToUse);
...
describe("AI Credits Mutations", () => {
  ...
  describe("useCredits Mutation", () => {
    ...
    test("should deduct credits and record usage", async () => {
      const result = await asUser.mutation(api.aiCredits.mutations.useCredits, {
        type: usageType,
