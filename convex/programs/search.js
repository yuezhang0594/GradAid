    ...
    handler: async (ctx, args) => {
        ...
        if (searchQuery && searchQuery.trim()) {
            ...
            const searchedPrograms = await ctx.db.query("programs")
                .withSearchIndex("search_programs", (q) => q.search("name", searchQuery))
                .collect();
            ...
            if (searchedPrograms.length > 0) {
                programsQuery = programsQuery.filter(q => q.or(...searchedPrograms.map(p => q.eq(q.field("_id"), p._id))));
            }
            if (searchedUniversities.length > 0) {
                universitiesQuery = universitiesQuery.filter(q => q.or(...searchedUniversities.map(u => q.eq(q.field("_id"), u._id))));
            }
        }
        ...
        if (filters.ranking && filters.ranking !== "all") {
            ...
            if (operator === "top" && !isNaN(rankingValue)) {
                universitiesQuery = universitiesQuery.filter((q) => q.lte(q.field("ranking"), rankingValue));
            }
        }
        // Apply location filter
        if (filters.location && filters.location !== "all") {
            const [filterCity, filterState] = filters.location.split(", ");
            universitiesQuery = universitiesQuery.filter((q) => q.and(q.eq(q.field("location.city"), filterCity), q.eq(q.field("location.state"), filterState)));
        }
        ...
        programsQuery = programsQuery.filter((q) => q.or(...universities.map((university) => q.eq(q.field("universityId"), university._id))));
        // Program type filter
        if (filters.programType && filters.programType !== "all") {
            programsQuery = programsQuery.filter((q) => q.eq(q.field("degree"), filters.programType));
        }
        // GRE requirement filter (user selects "GRE not required")
        if (filters.gre === true) {
            programsQuery = programsQuery.filter((q) => q.eq(q.field("requirements.gre"), false));
        }
        // TOEFL requirement filter (user selects "TOEFL not required")
        if (filters.toefl === true) {
            programsQuery = programsQuery.filter((q) => q.eq(q.field("requirements.toefl"), false));
        }
        // Minimum GPA filter
        if (filters.minimumGPA !== undefined) {
            programsQuery = programsQuery.filter((q) => q.or(q.eq(q.field("requirements.minimumGPA"), null), q.gte(q.field("requirements.minimumGPA"), filters.minimumGPA)));
        }
...
export const getUniversitiesForPrograms = query({
    ...
    handler: async (ctx, args) => {
        ...
        const universitiesQuery = ctx.db
            .query("universities")
            .filter(q => q.or(...Array.from(uniqueUniversityIds).map(id => q.eq(q.field("_id"), id))));
...
export async function getProgramsByUniversityId(ctx, universityId) {
    const programs = await ctx.db.query('programs')
        .filter(q => q.eq(q.field('universityId'), universityId))
        .collect();
    return programs;
}
export async function getUniversityIdByProgramID(ctx, programId) {
    const program = await ctx.db.get(programId);
    return program?.universityId || null;
}
export async function getUniversityById(ctx, universityId) {
...
export async function getUniversitiesHelper(ctx, universityIds) {
