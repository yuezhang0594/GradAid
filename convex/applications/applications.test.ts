...
describe("Application Model Functions", () => {
  ...
  beforeEach(async () => {
    ...
    [applicationId1, applicationId2, docId1, docId2] = await t.run(
      async (ctx) => {
        ...
        // Add a completed doc for progress testing
        ...
        return [appId1, appId2, dId1, dId2];
      }
    );
  });
    ...
    test("should throw error if application not found", async () => {
      ...
      await expect(
        asUser.run(async (ctx) => {
          await ApplicationModel.verifyApplicationOwnership(ctx, nonExistentId);
        })
  ...
  describe("validateProgramBelongsToUniversity", () => {
    ...
    test("should throw error if program does not belong to university", async () => {
      ...
      ).rejects.toThrow("Program does not belong to the specified university");
  ...
  describe("getApplicationDocumentsByUniversity", () => {
    test("should return documents grouped by university", async () => {
      ...
      const testUniData = groupedDocs.find((u) => u.name === "Test University");
      ...
      const docs = await t.run(
        async (ctx) =>
          await ctx.db
            ...
            .withIndex("by_application", (q) => q.eq("applicationId", newAppId))
            .collect()
      );
  ...
  describe("getApplicationsForUser", () => {
    ...
    test("should return empty array for user with no applications", async () => {
      const apps = await t.run(async (ctx) => {
        return await ApplicationModel.getApplicationsForUser(ctx, otherUserId);
      });
      expect(apps).toEqual([]);
    });
  });
...
describe("Application Queries", () => {
  ...
  let docId1: Id<"applicationDocuments">;
  let docId2: Id<"applicationDocuments">;
  let completedDocId: Id<"applicationDocuments">; // ID for the completed doc

  const clerkId = `clerk-query-test-${Date.now()}-${Math.random()}`;
  const otherClerkId = `clerk-other-query-${Date.now()}-${Math.random()}`;
  const asUser = t.withIdentity({ subject: clerkId });
  const asOtherUser = t.withIdentity({ subject: otherClerkId });
  const deadline1 = new Date(
    Date.now() + 10 * 24 * 60 * 60 * 1000
  ...
  // Re-use setup from model tests, but scoped to this describe block
  beforeAll(async () => {
    [
      testUserId,
      otherUserId,
      ...
      const otherId = await ctx.db.insert("users", {
        ...
        email: `other-query-${Date.now()}@example.com`,
        clerkId: otherClerkId,
      });
      const uniId = await ctx.db.insert("universities", {
        name: "Test University",
  ...
  beforeEach(async () => {
    ...
    [applicationId1, applicationId2, docId1, docId2, completedDocId] =
      await t.run(async (ctx) => {
        const appId1 = await ctx.db.insert("applications", {
          userId: testUserId,
          universityId: universityId,
  ...
  describe("getApplications", () => {
    test("should return applications with progress for the authenticated user", async () => {
      const apps = await asUser.query(api.applications.queries.getApplications);
  ...
  describe("getApplication", () => {
    test("should return the application if owned by the user", async () => {
      const app = await asUser.query(api.applications.queries.getApplication, {
        applicationId: applicationId1,
      });
...
describe("Application Mutations", () => {
  ...
  const clerkId = `clerk-mutation-test-${Date.now()}-${Math.random()}`;
  const otherClerkId = `clerk-other-mutation-${Date.now()}-${Math.random()}`;
  const asUser = t.withIdentity({ subject: clerkId });
  const asOtherUser = t.withIdentity({ subject: otherClerkId });
  const deadline1 = new Date(
    Date.now() + 10 * 24 * 60 * 60 * 1000
  ...
  // Re-use setup, similar to Queries block
  beforeAll(async () => {
    [
      testUserId,
      otherUserId,
      ...
      const userId = await ctx.db.insert("users", {
        ...
        email: `mutation-test-${Date.now()}@example.com`,
        clerkId: clerkId,
      });
      const otherId = await ctx.db.insert("users", {
        name: "Other Mutation User",
        email: `other-mutation-${Date.now()}@example.com`,
        clerkId: otherClerkId,
      });
      const uniId = await ctx.db.insert("universities", {
        name: "Test University",
...
describe("Application Timeline", () => {
  ...
  const clerkId = `clerk-timeline-test-${Date.now()}-${Math.random()}`;
  const otherClerkId = `clerk-other-timeline-${Date.now()}-${Math.random()}`;
  const asUser = t.withIdentity({ subject: clerkId });
  const asOtherUser = t.withIdentity({ subject: otherClerkId });
  const deadline1 = new Date(
    Date.now() + 10 * 24 * 60 * 60 * 1000
  ...
  beforeAll(async () => {
    [
      ...
      const otherId = await ctx.db.insert("users", {
        name: "Other Timeline User",
        email: `other-timeline-${Date.now()}@example.com`,
        clerkId: otherClerkId,
      });
      const uniId = await ctx.db.insert("universities", {
        name: "Test University",
  ...
  beforeEach(async () => {
    ...
    [applicationId1, applicationId2] = await t.run(async (ctx) => {
      const appId1 = await ctx.db.insert("applications", {
        userId: testUserId,
        universityId: universityId,
        programId: programId1,
  ...
  test("getTimeline: should return applications sorted by deadline (earliest first)", async () => {
    const timeline = await asUser.query(api.applications.timeline.getTimeline);
  ...
  test("getTimeline: should return empty array for user with no applications", async () => {
    const timeline = await asOtherUser.query(api.applications.timeline.getTimeline);
